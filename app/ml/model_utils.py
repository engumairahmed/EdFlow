...
def get_trained_models_summary():
    ...
    try:
        ...
        current_app.logger.info(f"Retrieved summary for {len(models_summary)} trained models.")
    except Exception as e:
...
def get_classification_models_summary():
    ...
    try:
        model_groups = db.trained_models.find({}).sort('created_at', -1)

        for group in model_groups:
            dataset_name = group.get('dataset', 'Unknown Dataset')
            ...
            for detail in group.get('details', []):
                if detail.get('type') == 'classification':
                    ...
                    models_summary.append({
                        ...
                        'trained_at': group.get('created_at', datetime.min.replace(tzinfo=timezone.utc))
                    })

    except Exception as e:
        current_app.logger.error(f"Error retrieving classification models summary: {e}", exc_info=True)
